...
| `test-skipped`                   | **High**   | Added lines in test files that skip or focus tests: `it.skip`, `.only`, `xit`, `fdescribe`, `test.fixme`, `pytest.mark.skip`/`xfail`, `t.Skip`, `@Disabled`, `#[ignore]`. Skips conditioned only on the platform, such as `it.skipIf(process.platform === \"win32\")`, are allowed                                                                                               |
